...
const VERSION = "scrolller-test v4 (reqUrl/reqRequest 修复)";
...
async function scrolllerFetch(sub, filter, limit, reqUrl, reqRequest) {
  ...
  try {
    ...
    // Scrolller 后端会校验 Origin/Referer 头，否则返回 404
    // 优先用浏览器请求头里的 Origin/Referer，回退用 API 域名
    const reqOrigin = reqRequest && reqRequest.headers ? reqRequest.headers.get("origin") : "";
    const reqReferer = reqRequest && reqRequest.headers ? reqRequest.headers.get("referer") : "";
    const pageOrigin = reqUrl ? reqUrl.origin + reqUrl.pathname : "https://api.scrolller.com";
    headers["Origin"] = reqOrigin || "https://api.scrolller.com";
    headers["Referer"] = reqReferer || pageOrigin;
...
async function introspect(reqUrl, reqRequest) {
  const queries = {
  ...
  // 共享请求头（与 scrolllerFetch 一致：带 Origin/Referer 防 404）
  const reqOrigin = reqRequest && reqRequest.headers ? reqRequest.headers.get("origin") : "";
  const reqReferer = reqRequest && reqRequest.headers ? reqRequest.headers.get("referer") : "";
  const pageOrigin = reqUrl ? reqUrl.origin + reqUrl.pathname : "https://api.scrolller.com";
  const baseHeaders = {
    "Content-Type": "application/json",
    "User-Agent": UA,
    "Origin": reqOrigin || "https://api.scrolller.com",
    "Referer": reqReferer || pageOrigin
  };
  const result = {};
...
<div style="max-width:820px;margin:0 auto">
  ...
  <div style="font-size:12px;color:#888;margin-bottom:4px">CF 服务端实测 = 阅读器实际播放路径，结果权威</div>
  <div style="font-size:11px;color:#0a7d34;background:#e6f4ea;padding:4px 8px;border-radius:4px;display:inline-block;margin-bottom:16px">📦 当前运行版本：${escapeHtml(VERSION)}</div>
...
export async function onRequest({ request }) {
  ...
  const api = await scrolllerFetch(params.sub, params.filter, params.limit, url, request);
  ...
  const intro = params.introspect ? await introspect(url, request) : null;
